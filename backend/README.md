# 🌐 JSD-MONO Backend: คู่มือและสถาปัตยกรรมระบบการเรียนรู้ API (API Learning Project)

> สำหรับเอกสารฉบับสมบูรณ์ที่ครอบคลุมทั้งโปรเจค สามารถดูได้ที่ไฟล์ [Root README.md](../README.md)

ยินดีต้อนรับสู่ระบบ **Backend RESTful API** ของโปรเจค `JSD-MONO` พัฒนาด้วย Node.js และ Express แสดงวิวัฒนาการตั้งแต่ระดับพื้นฐานจำลองฐานข้อมูลในหน่วยความจำ (In-Memory Database) จนถึงระบบที่เชื่อมต่อฐานข้อมูลจริงทั้ง **MongoDB (Mongoose ODM)** และ **Supabase (PostgreSQL)** พร้อมระบบความปลอดภัย **Bcrypt Hashing**, **JWT Authentication** และ **HttpOnly Cookie**
...
| :--- | :--- | :--- | :--- | :--- |
| **REST (Representational State Transfer)** *(โปรเจคนี้ใช้เป็นหลัก)* | สื่อสารแบบ Stateless ผ่าน HTTP Methods มาตรฐาน อิงทรัพยากร (Resources) ตาม URI | HTTP/HTTPS (JSON, XML, Text) | ง่าย, เป็นมาตรฐานสากล, มี Cache ในตัว, ยืดหยุ่นสูง | เว็บแอปพลิเคชันทั่วไป, Mobile Apps, CRUD APIs |
...
| `DELETE /api/v2/users/pg/:id` | `.from("users").delete().eq("id", id)` | ลบแถวข้อมูลในตาราง SQL ของ Supabase ตาม Primary Key ID |
...
2. สร้างไฟล์ `.env` ที่โฟลเดอร์ `backend/` และระบุค่าตัวแปร:
   ...
   ```
   *(หมายเหตุ: สามารถรัน `node src/utils/generateSecretKey.js` เพื่อสุ่มสร้าง `JWT_SECRET` ที่ปลอดภัยได้)*
...
เปิดไฟล์ `.rest` ผ่าน Visual Studio Code (ที่ติดตั้ง Extension *REST Client*):
